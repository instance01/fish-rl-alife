...
axs[0].set_ylabel(r'Herding Ratio $\eta$', fontsize=11)
axs[0].set_xlabel(r'Initial predator survival time t $\times$ 100', fontsize=11)
x = [str(int(x[1:]) // 100) for x in i10_p150[0]]
y = [x[0] for x in i10_p150[1]]
# axs[0].bar(x, y, color='#009440')
axs[0].bar(x, y, color=col1)
x = np.arange(len(y))
...
axs[1].set_xlabel(r'Initial predator survival time t $\times$ 100', fontsize=11)
x = [str(int(x[1:]) // 100) for x in i5_p150[0]]
y = [x[0] for x in i5_p150[1]]
# axs[1].bar(x, y, color='#009440')
axs[1].bar(x, y, color=col1)
x = np.arange(len(y))
fittedParameters, pcov = curve_fit(func, x, y)
y_fit = func(x, *fittedParameters)
...
axs[2].set_xlabel(r'Initial predator survival time t $\times$ 100', fontsize=11)
x = [str(int(x[1:]) // 100) for x in i2_p150[0]]
y = [x[0] for x in i2_p150[1]]
# axs[2].bar(x, y, color='#009440')
axs[2].bar(x, y, color=col1)
x = np.arange(len(y))
fittedParameters, pcov = curve_fit(func, x, y)
y_fit = func(x, *fittedParameters)
y_fit[1] -= .01
