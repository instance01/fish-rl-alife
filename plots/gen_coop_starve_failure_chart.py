...
def _prep(data, prefix='i10'):
    ...
    for k, v in zip(data[0], data[1]):
        ...
        # ret[k] = failures
        # ret_labels[k] = '%0.2f' % failures
        ret[k] = failures
        ret_labels[k] = '%0.2f\n±%0.2f' % (round(failures, 2), round(failures_ci, 2))
    print(ret_labels)
...
def doit_single(id_, i5_data, i5_label_data, i5_data2, i5_label_data2, i5_data3, i5_label_data3, i5_data4, i5_label_data4):
    ...
    # cmap_mod = truncate_colormap('Greens', minval=.3, maxval=.99)
    # cmap_mod = truncate_colormap('Blues', minval=.5, maxval=.99)
    ...
    for m in range(4):
        ...
        if m <= 1:
            ...
            ax.yaxis.set_label_coords(-.17, -.23)
        ...
        fig.savefig(id_ + "_coop_starve_failure" + str(m) + ".pdf", bbox_inches='tight')
        plt.show()
